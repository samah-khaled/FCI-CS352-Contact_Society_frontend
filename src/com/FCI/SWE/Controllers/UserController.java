...
public class UserController {
	...
	public void search(String name) {
		new Connection().execute(
				"http://dotted-marking-88320.appspot.com//SearchService",name,
				 "SearchService");
	}
		...
		protected void onPostExecute(String result) {
			...
			try {
				...
				if (serviceType.equals("LoginService")) {
					...
					Intent homeIntent = new Intent(Application.getAppContext(),
							HomeActivity.class); //ShowSearch.class
					homeIntent.setFlags(Intent.FLAG_ACTIVITY_NEW_TASK);
				
         JSONArray items=  object.getJSONArray(result); // result not sure 
        for (int i = 0; i < items.length(); i++) {
            JSONObject obj = items.getJSONObject(i);
            users.add(
                    new UserEntity(obj.getString("name"), 
                        obj.getString("email"), 
                        obj.getString("password")));
            
        }
                       homeIntent.putExtra("user",users);
